...
select genre, avg(price) as average_price from books group by genre having avg(price) > 14;
-- having clause is used for conditionals on aggregate functions
-- Find all customers whose email addresses end with .com and are from either USA or UK
select * from customers 
where email like '%.com' and (country in ('USA', 'UK'));
...
select upper(concat(customer_first_name, ' ', last_name)) as full_name, email, lower(city) as city_lowercase from customers
